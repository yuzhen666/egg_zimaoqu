...
class UserService extends Service {
    // async getAll(param) {
    //     if (param.region == 'NO') {
    //         const results = await this.app.mysql.select('total', {
    //             orders: ['report_id'],
    //             limit: 10,
    //             offset: 10 * (param.pageNum || 1 - 1),
    //         });
    //         const totalNum = await this.app.mysql.select('total', {
    //             orders: ['report_id'],
    //         });
    ...
    //     } else {
    //         const results = await this.app.mysql.select('total', {
    //             where: { report_region: param.region },
    //             orders: ['report_id'],
    //             limit: 10,
    //             offset: 10 * (param.pageNum || 1 - 1),
    //         });
    ...
    async getAll(param) {
        ...
        if (param.region == 'all') {
            res = await this.app.mysql.query(
                // `select id, region, date, url, title, abstract from zimaoqu where region = '${param.region}' and abstract like '%${param.keywords}%'  and date < ${newDate}  and id limit ` + ((param.pageNum || 1 - 1) * 10) + "," + 10,
            ...
            totalNum = res.length;
            results = res.slice(10 * (param.pageNum), 10 + 10 * (param.pageNum));
            return { results, totalNum };
        } else {
            res = await this.app.mysql.query(
                // `select id, region, date, url, title, abstract from zimaoqu where region = '${param.region}' and abstract like '%${param.keywords}%'  and date < ${newDate}  and id limit ` + ((param.pageNum || 1 - 1) * 10) + "," + 10,
